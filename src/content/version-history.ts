...
export const VERSION_HISTORY: VersionEntry[] = [
  {
    ...
    changes: {
      added: [
        ...
        '/open — the work in the open: a published surface for the live working material (applications, evaluations, the Six-Hat method, and the register of what is next), each rendered with its uncertainty on its face. The counterpart to retiring drafts: not a backstage queue but the thinking itself, published as it happens. First contents: the Ogwen DNA-economy application and its full six-hat evaluation, the evaluation method, the explanation-layer direction, and the backlog.',
        'cleanliness-and-scale (forming) — generalised out of the Saudi-football-legal six-hat run (/open): where a large flow of value is tied to credible harm, the amount you can take and the cleanliness of taking it are inversely correlated, because your own legitimacy is the product the harm buys — so sincerity launders more efficiently than cynicism. The clean configuration exists but is small and adversarial (profit by resisting the harm, not receiving its coin). Carries the four-test line (counterparty / forum / brand-spend / indivisibility) that separates floor-raising from laundering; ships with its adversary (the abandon-the-field objection).',
        'route-around-the-chokepoint (forming) — generalised out of the Ogwen evaluation (an application throwing off a principle): the binding boundary on a reachable configuration is often a control chokepoint (the regulated grid, where electrical and political power coincide), not a physical limit; the move is to architect at the largest scale that stays below the chokepoint (building-and-cluster, behind the meter) and own it collectively — else it is the comfortable seceding to private abundance. The control-chokepoint corollary of coordination-bounds-reachability; ships with its adversary (the secession objection; inter-cell equity unresolved).',
